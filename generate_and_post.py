...
// Runs "The Off-Ramp" four-audience campaign (payors, employers, TPAs, brokers).
// Campaign mode (default): 12 single posts; 2 carousels in segment phase, 0 in broad phase.
// Env CAMPAIGN_PHASE=broad selects Phase 1; default is segment. CAMPAIGN_ACTIVE=false
// reverts to the standard 5 payor + 5 employer + 2 employer-carousel run.
// Pushes single posts to Publer as drafts, writes carousel scripts to out/, and appends
// to ledger.json so angles never repeat across runs. Organic content layer only; the paid
// buy (Insight Tag, Matched Audiences, retargeting, Lead Gen Forms) runs in Campaign Manager.
//
...
// ---- CAMPAIGN: "The Off-Ramp" (four-audience, two-phase) ----
// Driven by env so workflow_dispatch can pick the phase without editing code:
//   CAMPAIGN_ACTIVE = "false" reverts to the standard two-audience weekly run.
//   CAMPAIGN_PHASE  = "broad" for Phase 1 problem-awareness; anything else = "segment".
// Kept in sync with SKILL.md.
const CAMPAIGN = {
  active: (process.env.CAMPAIGN_ACTIVE ?? "true") !== "false",
  phase: process.env.CAMPAIGN_PHASE === "broad" ? "broad" : "segment",
};
const SINGLE_MIX = CAMPAIGN.active
  ? { payors: 3, employers: 3, tpas: 3, brokers: 3 } // 12 single posts
  : { payors: 5, employers: 5 };
const CAROUSEL_MIX = CAMPAIGN.active
  ? (CAMPAIGN.phase === "broad" ? {} : { employers: 1, brokers: 1 })
  : { employers: 2 };
const BATCH_SIZE = 5;
...
const AUDIENCES = {
  ...
  tpas: {
    ...
    who: "Presidents/COOs, VP Client Services, Directors of Claims, VP Product/Network Solutions, Chief Growth Officers, and VP Care/Utilization Management at TPA firms.",
    pains: "Every renewal season they must explain a behavioral health trend they cannot control, with no outcome data to defend the spend. Differentiation against competing TPAs depends on the point-solution stack they bring to clients, and behavioral health is the line clients ask about that they cannot answer well.",
    valueProps: "A plug-in point solution that drops into existing plan administration with no plan redesign; a stickiness play, since clients do not leave the administrator who solved their worst claim category; a new-business weapon for finals where every competing TPA sounds identical; machine-readable outcome data across six domains that upgrades their client reporting at zero build cost.",
    language: "Renewal conversation, trend, point-solution stack, client reporting, stickiness, finals, carve-out. Partner-first, make-them-look-good-to-their-client framing.",
    pillarsPreferred: ["Payor ROI", "Employer Benefits", "Thought Leadership", "Neurodiversity Awareness"],
    cta: "See how BrainyAct plugs into your book at brainyact.com/for-businesses, or book a 20-minute partnership walkthrough at meetings.hubspot.com/macy-hanson/macy.",
    numericPolicy: "Same as employers: approved external stats (always attributed), approved product facts, and the one approved cost model. NO internal outcome figures. NO guarantee language. NO invented outcome numbers (never a specific 'down X%' renewal figure). Critique the ABA reimbursement model, never ABA clinically.",
  },
  brokers: {
    label: "Brokers and Benefits Consultants",
    who: "Voluntary benefits leaders, specialty practice leaders, health and wellbeing consultants, employee benefits consultants and producers, and SVP Client Management at broker and consulting firms.",
    pains: "They compete for self-insured RFPs with near-identical stop-loss, pharmacy, and point-solution stories. They already sell autism coverage but have nothing new to say about the cost side of it, and they win by teaching the client something about their own plan the other firms did not.",
    valueProps: "RFP differentiation through an outcome-measured, fixed-duration behavioral health point solution no competing firm carries yet; a CFO-ready savings model the consultant can present under their own advisory brand; complementary positioning that fills the ABA waitlist gap without arguing against a benefit they recommended; deal support (ROI sessions, pilot frameworks, co-presented meetings) so the consultant carries zero delivery risk.",
    language: "RFP, finals table, advisory story, cost-containment, point solution, own the category, savings model. Advisor-first, be-the-smartest-in-the-room framing.",
    pillarsPreferred: ["Employer Benefits", "Payor ROI", "Thought Leadership", "Neurodiversity Awareness"],
    cta: "Register for the BrainyAct broker ROI roundtable (peer format, real math, no pitch deck); we send the date on sign-up. Or book a walkthrough at meetings.hubspot.com/macy-hanson/macy.",
    numericPolicy: "Same as employers: approved external stats (always attributed), approved product facts, and the one approved cost model. NO internal outcome figures. NO guarantee language. NO 'outcome guarantee' as an RFP selling point. Critique the ABA reimbursement model, never ABA clinically.",
  },
};
...
const TRAINING = {
  ...
  tpas: {
    examples: [],
    facts: [
      "TPAs sit between the math and the client. The renewal moment is the whole story: the client points at the behavioral health line, asks why it grew again, and the TPA has no answer that does not sound like an excuse.",
      "The play is bring the client a fix plus the reporting that makes them look brilliant, stickiness (clients do not leave the administrator who solved their worst claim line), and a finals differentiator.",
      "BrainyAct is a plug-in point solution: a carve-out or navigation alternative, no plan redesign, machine-readable outcome data across six domains that flows into the TPA client reporting.",
    ],
    requires: ["Partner-first framing: make the TPA look good in front of THEIR client", "One idea per line", "Open on the renewal-conversation tension or a clean reframe", "Attribute every external stat", "Before the soft CTA, ask one genuine question (e.g. 'What do you say when a client asks why behavioral health grew again?')", "Soft CTA to the partnership walkthrough"],
    bans: ["Any guarantee or 'performance guarantee' language", "Invented outcome numbers, including any specific 'down X%' renewal-reduction figure", "Internal outcome percentages and participant counts", "The $150K/$43M cost version", "Attacking ABA clinically", "Hashtag stuffing (max 5)"],
  },
  brokers: {
    examples: [],
    facts: [
      "Brokers win RFPs and renewals by teaching the client something about their own plan the other firms did not. Autism cost is an unclaimed advisory story: 54% of employers cover ABA (Gallagher 2025) and almost none get outcome data for it.",
      "The finals table is the emotional center: three firms, three near-identical decks, and the consultant who opens with the open-ended-claim problem plus the fixed-duration alternative is the one they remember.",
      "BrainyAct gives the consultant a differentiated point solution no competing firm carries yet, a CFO-ready savings model they can present under their own brand, and complementary positioning that fills the ABA waitlist gap.",
    ],
    requires: ["Advisor-first framing: help the consultant look like the smartest in the room", "One idea per line", "Open on the finals-table tension, the unclaimed-category reframe, or their own benchmark data reflected back", "Attribute every external stat", "Before the soft CTA, ask one genuine question (e.g. 'What are you walking into the RFP with that the other firms are not?')", "Soft CTA to the roundtable registration; invite them to register, do not name a date"],
    bans: ["'Outcome guarantee' or any guarantee language as a selling point", "Invented numbers", "Internal outcome percentages and participant counts", "The $150K/$43M cost version", "Naming a fixed roundtable date (there is none yet; it goes out on sign-up)", "Attacking ABA clinically", "Hashtag stuffing (max 5)"],
  },
};
...
// ---- "THE OFF-RAMP" CAMPAIGN COPY (scrubbed of banned claims) ----
// Dropped from the source doc: "$15M or $2M. Same 100 kids." ($150K/member = banned)
// and every "performance guarantee" line.
const CAMPAIGN_BROAD_HOOKS = [
  "Every claim in your plan has an endpoint. Except this one.",
  "Nobody is gaming the system. The system just has no exit.",
  "Surgery has discharge criteria. Cardiac rehab has an end date. Why doesn't behavioral health?",
  "You know what you paid. Do you know if it worked?",
  "For every other condition, care ends when the patient gets better. For these kids, care just continues.",
  "What if the claims stopped because the kids actually got better?",
];
const CAMPAIGN_BROAD_FRAMES = [
  "The honest mirror: we pay, kids get services, the claims keep coming, and nobody asks if it is working. No accusation, just the sentence every benefits leader has thought and never said.",
  "Paying twice: when a child is not getting better, the employer pays once through the plan and once through the parent who cannot fully show up to work.",
  "The exhausted family: families are not demanding too much. They are stuck in a revolving door of referrals, waitlists, and services with no end date. Empathy first.",
];
const CAMPAIGN_BROAD_CTA = "See the math at brainyact.com/for-businesses.";
const SEGMENT_TAGLINES = {
  payors: ["Stop managing utilization. Start measuring outcomes.", "The alternative to the authorization fight.", "Give members somewhere to go, instead of something to deny.", "Your network has a 12 to 18 month waitlist. We start at home this week."],
  employers: ["You are self-insured. Every one of these dollars is yours.", "The line item that renews itself, until now.", "You do not have to wait for renewal to fix this.", "Your plan pays for the therapy. Your payroll pays for the burnout."],
  tpas: ["Stop explaining the spend. Start showing the outcome.", "Your client's hardest renewal conversation just got an answer.", "Be the TPA that solved the category everyone else just reports on.", "Clients do not leave the administrator who fixed their worst claim line."],
  brokers: ["Walk into the RFP with the point solution nobody else brought.", "Every consultant has a cost-containment story. Yours can have a finish line.", "Your clients cover ABA. None of them can tell you if it is working. Change that.", "Be the advisor who found the off-ramp first."],
};
...
- Payors: fully qualitative. No numbers at all, ever, even in the campaign's problem phase.
- Employers, TPAs, and Brokers: you MAY use the approved external stats (always attributed), approved product facts, and the single approved cost model below. Nothing else numeric.
${APPROVED_STATS}
${COST_MODEL}
- BANNED for everyone: the 91% figure and any internal outcome percentage or participant count; the $150K/$43M cost version (this includes "$15M per 100"); ANY performance or outcome guarantee, including "measurable progress or we continue at no cost" and "outcome guarantee" as an RFP selling point; invented numbers such as a specific "down X%" renewal reduction; "proven", "rewire", "cure", "treatment" as an efficacy claim.
...
function buildBatchPrompt(audienceKeys, ledger) {
  const broad = CAMPAIGN.active && CAMPAIGN.phase === "broad";
  const fmts = shuffle(FORMATS), hooks = shuffle(HOOKS);
  const bHooks = shuffle(CAMPAIGN_BROAD_HOOKS), bFrames = shuffle(CAMPAIGN_BROAD_FRAMES);
  ...
  const blocks = audienceKeys.map((key, i) => {
    ...
    if (broad) {
      return `Post ${i + 1}
  Audience: ${a.label} (write so this segment recognizes itself, but do NOT pitch the product)
  Their pains: ${a.pains}
  Campaign anchor line to open from (use or riff, do not copy verbatim every time): "${bHooks[i % bHooks.length]}"
  Emotional frame to build the body on: ${bFrames[i % bFrames.length]}
  Length: ${lengths[i]} (${lengths[i] === "short" ? "120-200" : "220-350"} words)
  CTA: ${CAMPAIGN_BROAD_CTA}
  numericPolicy: NO pricing and NO product features in Phase 1. If this audience is payors, stay fully qualitative with NO numbers at all. For non-payors you MAY reference ONE approved, attributed external stat if it sharpens the problem, but nothing about BrainyAct's own results.
  Hard rules: Name the problem honestly, absolve the families (nobody is gaming the system), and plant the idea that a finish line exists. No feature list. No cost model. No guarantee. End on the soft CTA and 3 to 5 hashtags led by #BrainyAct #Neurodiversity #DigitalTherapeutics.
  Must avoid: ${(t.bans || []).join("; ") || "n/a"}`;
    }
    const seg = (CAMPAIGN.active && SEGMENT_TAGLINES[key]) ? shuffle(SEGMENT_TAGLINES[key])[0] : "";
    return `Post ${i + 1}
  Audience: ${a.label}
  Who they are: ${a.who}
  Their pains: ${a.pains}
  ...
  Hook style: ${hooks[i % hooks.length]}${seg ? `\n  Segment angle seed (build toward this idea, do not paste it as-is): "${seg}"` : ""}
  ...
  const phaseNote = broad
    ? `\nThis is PHASE 1 (broad problem-awareness) of "The Off-Ramp". Every post names the one shared problem (autism/ADHD behavioral health is the only major claim category with no endpoint, no outcome data, no off-ramp) and stays blame-free toward families. No product, no pricing, no offer.`
    : (CAMPAIGN.active ? `\nThis is PHASE 2 (segment answer) of "The Off-Ramp". Each post serves one audience its own ROI story and offer.` : "");
  return `Generate exactly ${audienceKeys.length} LinkedIn single posts. Each must read as structurally different from every other item.${phaseNote}
...
async function main() {
  ...
  await writeFile(new URL(`./out/carousels-${stamp}.md`, import.meta.url), `# Carousels — ${stamp}\n\n${md}`);
